...
function renderCards() {
  ...
  items.forEach(item => {
    ...
    card.innerHTML = `
      <img src="${defaultImg}${revision}" alt="${item.name}" loading="lazy" width="832" height="1216" onerror="this.onerror=null; this.src='/${item.id}/01.webp';">
...
function renderModalVariants(item) {
  ...
  visibleVariants.forEach(v => {
    ...
    itemEl.innerHTML = `
      <img src="${imgPath}" alt="${v}" loading="lazy" onerror="this.onerror=null; this.src='/${item.id}/01.webp';">
...
function updateModalPreview() {
  ...
  if (el.modalTagInput) {
    el.modalTagInput.value = markdownTag;
  }
}
