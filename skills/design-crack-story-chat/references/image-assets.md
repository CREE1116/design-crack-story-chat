...
> **접두사는 `s`=safe, `a`=adult 다.** 이전 판 예시가 `a`=평상, `s`=19+ 로 거꾸로 적혀 있었다. 전연령을 `s01` 부터, 성인을 `a01` 부터 세고 `s00` 은 명함으로 비워 둔다. 배경은 인물 축이 아니므로 `bg01` 을 쓰며, 크롭 도구의 `--style scene` 이 붙이는 `a01` 은 이 규약과 어긋나므로 `--naming` 으로 `bg` 접두사를 지정한다.

토큰 효율과 씬 분류를 위해 인물은 **2자리 숫자 ID**로 치환하고, 파일명은 **`s`(safe·전연령) / `a`(adult·성인)** 접두사를 부여하여 `.webp` 포맷으로 관리합니다.
...
1. **무작위 파일명 사용 금지**: `image1.png`, `temp.jpg` 등 축과 무관한 파일명 배제. 반드시 `{id}/s01.webp`(전연령) 또는 `{id}/a01.webp`(성인) 사용. `s00` 은 명함으로 예약한다.
