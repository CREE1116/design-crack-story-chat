...
- Decision points are **rare rather than per-response**. A rule that says how to close a decision beat, without saying that most responses are not decision beats, becomes a closing template: the menu disappears and the fork moves into prose as `A할 것인가 B할 것인가`. Enumerating options in narration is still enumeration. Most responses should move the world one step and stop, leaving the player free to react or not.
